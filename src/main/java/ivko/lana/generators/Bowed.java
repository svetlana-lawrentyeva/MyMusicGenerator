...
public class Bowed
{
    ...
    {
        return generateSamples(baseFrequency, duration, 1, 1);
    }

    // Метод для генерации звука с более мягкими гармониками и фильтрами
    public short[] generateSamples(double baseFrequency, double duration, double modulationStepMs, double modulationStepHz)
    {
        ...
        for (int i = 0; i < numSamples; i++)
        {
            ...
            double modulation = Math.sin(2.0 * Math.PI * modulationStepHz * i / Constants.SAMPLE_RATE) * modulationStepMs;
            // Постоянное вибрато
//            vibratoGain = 0;
            double vibrato = Math.sin(2.0 * Math.PI * vibratoFrequency * i / Constants.SAMPLE_RATE) * vibratoGain * modulation;
